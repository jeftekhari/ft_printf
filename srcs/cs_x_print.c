...
/*   Updated: 2017/08/04 14:01:59 by jeftekha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int		cs_lc_x_wp(t_type *type, t_badge *badge, char **fmt, char *str)
{
	int			len;
	...
	if (badge->jleft)
	{
		...
		cs_lc_x_prec(type, badge, fmt, str);
...
int		cs_lc_x_prec(t_type *type, t_badge *badge, char **fmt, char *str)
{
	int		len;
...
int		cs_lc_x_width(t_type *type, t_badge *badge, char **fmt, char *str)
{
	int			len;
...
int		cs_lc_x_def(t_type *type, t_badge *badge, char **fmt, char *str)
{
	int		len;
...
char	*base_greater_10_x(t_type *type, int base)
{
	...
	while (nb > 0)
	{
		str[i] = nb % base > 9 ? nb % base + 87 : nb % base + 48;
		nb /= base;
		i--;
	}
	return (str);
